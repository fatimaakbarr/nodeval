...
export default function Sidebar({ activePage, setActivePage }) {
  ...
  return (
    <>
      ...
      <button
        className="md:hidden fixed top-4 left-4 z-50 w-10 h-10 rounded-xl flex items-center justify-center kinetic-gradient shadow-lg"
      ...
      {/* Sidebar — desktop: hover to expand | mobile: slide in */}
      <motion.aside
        ref={sidebarRef}
        className={[
          'h-screen flex flex-col py-8 bg-surface-container-low/80 backdrop-blur-xl border-r overflow-hidden relative z-40',
          // Mobile: fixed overlay, Desktop: static
          'fixed md:static',
          // Mobile: hide when closed
          mobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0',
        ].join(' ')}
        style={{ borderColor: 'rgba(172,163,255,0.08)', transition: 'transform 0.3s ease' }}
        animate={{ width: (hovered || mobileOpen) ? 220 : 72 }}
        transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
      >
        {/* Logo */}
        <div className="px-5 mb-10 flex items-center gap-3 overflow-hidden mt-8 md:mt-0">
          <motion.div
